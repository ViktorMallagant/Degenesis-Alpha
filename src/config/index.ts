...
export default {
  ...
  culturesByName: new Map(Object.values(Cultures).sort((a, b) => a.name.localeCompare(b.name, 'en')).map((c) => [c.name, c])),
  conceptsByName: new Map(Object.values(Concepts).map((c) => [c.name, c])),
  cultsByName: new Map(Object.values(Cults).sort((a, b) => a.name.localeCompare(b.name, 'en')).map((c) => [c.name, c])),
