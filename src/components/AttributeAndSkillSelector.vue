<template>
  ...
  <div v-for="skill in SkillsByAttribute.get(attribute) || []" v-bind:key="skill.name">
    <ValueSelector
      ...
      :value="store.effectiveSkillValue(skill)"
      :max="store.skillMax(skill) + store.legacySkillBonus(skill.name)"
      :min="skillMin() + store.legacySkillBonus(skill.name)"
      :active="store.isActiveSkill(skill)"
      @change="(v) => store.setSkill(skill, v - store.legacySkillBonus(skill.name))"
